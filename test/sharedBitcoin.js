...
const addressFromPublicKey = (publicKey, bitcoin) => {
  const pubkey = bitcoinUtil.compressPublicKey(Buffer.from(publicKey, 'hex'));
  if (bitcoin.ECPair.fromPublicKeyBuffer) { // bitcoinjs-lib version <= 3.3.2
    const keyPair = bitcoin.ECPair.fromPublicKeyBuffer(pubkey, bitcoin.networks.testnet);
    return keyPair.getAddress();
  }
  const pkh = bitcoin.payments.p2pkh({ pubkey, network: bitcoin.networks.testnet });
  return pkh.address;
};

const getP2PKHOutput = (publicKey, bitcoin) => {
  const pubkey = bitcoinUtil.compressPublicKey(Buffer.from(publicKey, 'hex'));
  if (bitcoin.ECPair.fromPublicKeyBuffer) { // bitcoinjs-lib version <= 3.3.2
    return bitcoinUtil.getDefaultScript(Buffer.from(publicKey, 'hex'));
  }
  const pkh = bitcoin.payments.p2pkh({ pubkey, network: bitcoin.networks.testnet });
  return pkh.output;
};
...
exports.test = function(bitcoin) {
  it('checks bitcoinjs and KeyChain signed transactions are equal', async () => {
    const getTransactionBuilder = (tx, unspents) => {
    ...
    const txParams = {
      from: addressFromPublicKey(publicKey, bitcoin),
    ...
    const txbKeychain = getTransactionBuilder(txParams, unspents);
    const tx = txbKeychain.buildIncomplete();
    // add input scripts to unsigned transaction https://github.com/bitcoinjs/bitcoinjs-lib/issues/1011#issuecomment-368394185
    unspents.forEach(({ scriptPubKey }, index) => tx.setInputScript(index, Buffer.from(scriptPubKey, 'hex')));
    ...
    const txbBitcoinJS = getTransactionBuilder(txParams, unspents);
  ...
  it('can create a 1-to-1 Transaction', async () => {
    ...
    txRaw.setInputScript(0, getP2PKHOutput(publicKey, bitcoin));
