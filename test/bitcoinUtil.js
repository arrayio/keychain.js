...
describe('bitcoin util', () => {
  it('has first byte equal to 03', () => {
    ...
    const compressedKey = bitcoinUtil.compressPublicKey(publicKey);
    const firstByte = compressedKey.readInt8(0);
    assert.strictEqual(firstByte, 3);
    assert.strictEqual(compressedKey.length, 33);
  });
  ...
  it('has first byte equal to 02', () => {
    ...
    const compressedKey = bitcoinUtil.compressPublicKey(publicKey);
    assert.strictEqual(compressedKey.readInt8(0), 2);
    assert.strictEqual(compressedKey.length, 33);
  });
});
